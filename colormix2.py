...
def pick_color1():
    ...
    palette_colors = ['#FF5733', '#33FF57', '#5733FF', '#FFD133', '#33FFF5', '#F533FF', '#33FFAA', '#AA33FF', '#FFAA33', '#AAFF33', '#33FF57', '#5733FF']

    # Create color buttons in the palette window
    for i, color in enumerate(palette_colors):
        tk.Button(palette_window, bg=color, command=lambda c=color: select_color_from_palette(c)).grid(row=i // 6, column=i % 6, padx=5, pady=5)
...
def select_color_from_palette(color):
    # Update the color entry with the selected color from the palette
    color1_entry.delete(0, tk.END)
    color1_entry.insert(0, color)
...
def select_color_from_palette(color):
    # Update color1 entry with the selected color from the palette
    color1_entry.delete(0, tk.END)
    color1_entry.insert(0, color)
    update_color()
...
palette_button = tk.Button(root, text="Open Color Palette", command=open_color_palette, bg="#009688", fg="white", font=("Helvetica", 10, "bold"))
palette_button.grid(row=3, column=0, padx=10, pady=5, sticky='nw')
...
update_button.grid(row=4, column=0, padx=(100,0), pady=(5,0), sticky='n')
