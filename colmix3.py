...
class ColorApp(tk.Tk):
    def __init__(self):
        ...
        self.mix_button = tk.Button(self, text="Mix the colors", command=self.mix_colors_button)
    ...
    def create_color_palette(self):
        ...
        self.colors = ['#FFB6C1', '#006400', '#ED7014', '#A020F0', '#FF0000', '#FFE5B4', '#FFFF00', '#90EE90', '#00008B', '#ADD8E6', '#964B00', '#000000']
    ...
    def set_selected_color(self, color):
        if self.color1 is None:
            ...
            # Reset mixed color if both colors are already selected
            self.mixed_color = None
            # Reset color1 and color2
            self.color1 = color
            self.color2 = None
            # Update color square
            self.canvas.itemconfig(self.color1_square, fill=self.color1)
            self.canvas.itemconfig(self.color2_square, fill="")
    ...
    def update_mixed_color(self):
        if self.color1 is not None and self.color2 is not None:
            ...
            self.mixed_color = mix_colors(self.color1, self.color2, percent1, percent2)
            self.canvas.itemconfig(self.mixed_color_square, fill=self.mixed_color)

    def mix_colors_button(self):
